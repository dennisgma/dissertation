...
cap program drop _isstr
program define _isstr , rclass
local str=0
if substr("`:type `1''" , 1, 3) == "str" {
	local str=1
}
...
program define _lgmreshape
	...
	//in case starting doesn't start at 1 
	forv t=1/6 {	
		if (`str'==0) {
			cap g `name'`t'=.
		}
	...
	//need to order in case there is a break:
	...
	while (`tp'<=`tpmax') {
		...
		forv x=1/`mpr' {
			...
			cap replace `name'`x'=`name'`add' in `pos'		//pull back all values accordin to tp_s
		}
		local pos=`pos'+1
		local tp=`tp'+`tpinc'
	}
...
foreach f of local todo {
	...
	//first, keep data
	//preserve
	save lgmrs_temp.dta,replace
	//next loop through the variables to store a temporary file for each
	qui ds
	local vlist "`r(varlist)'"
	local vlist: subinstr local vlist "`tpvar'" ""
	local vlist: subinstr local vlist "address" ""
	//local vlist: subinstr local vlist "_merge" ""	//not kept
	foreach v of local vlist {
		di "processing lgm reshape for `v'"
		...
		//restore
		use lgmrs_temp.dta, clear
	}	
	local first=0
	di "`vlist'"
	foreach v of local vlist {
		if (`first'!=0) {
			merge 1:1 tp_s using "`v'_temp.dta", nogen
		}
		else {
	...
	g address="`f'"
	save "`tofolder'/to_append/`f'_merged.dta" , replace	
	erase "lgmrs_temp.dta"
}
